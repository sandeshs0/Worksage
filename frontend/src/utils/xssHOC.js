  ...
  return function XSSProtectedComponent(props) {
    ...
    if (props.onChange) {
      securePropOverrides.onChange = (e) => {
        const sanitizedValue = sanitizeAndValidate(
          e.target.value,
          e.target.name || "unknown"
        );
        const secureEvent = {
          ...e,
          target: {
    ...
    if (props.defaultValue && typeof props.defaultValue === "string") {
      securePropOverrides.defaultValue = sanitizeAndValidate(
        props.defaultValue,
        "defaultValue"
      );
    }
    ...
    return (
      <div>
        ...
        <WrappedComponent {...props} {...securePropOverrides} />
      </div>
    );
  };
};
...
export const useXSSProtection = (options = {}) => {
  ...
  const protectInput = React.useCallback(
    (value, fieldName = "unknown") => {
      if (typeof value !== "string") return value;
  ...
  const protectedOnChange = React.useCallback(
    (originalOnChange) => {
      return (e) => {
  ...
  const protectedOnSubmit = React.useCallback(
    (originalOnSubmit) => {
      return (e) => {
        e.preventDefault();
