...
export const debugAuth = {
  ...
  simulateOAuthCallback(accessToken = "test-token", isNewUser = false) {
    console.log("🧪 Simulating OAuth callback with:", { accessToken, isNewUser });
    ...
    console.log("✅ OAuth simulation complete. Check console for GoogleAuthCallback logs.");
  },
  ...
  quickOAuthTest() {
    ...
    const dummyToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiJ0ZXN0LXVzZXItaWQiLCJ0eXBlIjoiYWNjZXNzIiwiaWF0IjoxNjAwMDAwMDAwLCJleHAiOjk5OTk5OTk5OTksImF1ZCI6IndvcmtzYWdlLXVzZXJzIiwiaXNzIjoid29ya3NhZ2UifQ.dummy-signature";
    ...
    console.log("✅ Dummy OAuth data set. Try refreshing or navigating to dashboard.");
  ...
  testOAuthPreprocessing() {
    ...
    // Simulate OAuth URL
    const testUrl = window.location.origin + "/OAuthCallback?accessToken=test-oauth-token&isNewUser=false";
  ...
  simulateOAuthCallback(token = "test-token") {
    ...
    window.history.pushState({}, '', `/OAuthCallback?accessToken=${token}&isNewUser=false`);
    
    console.log("✅ OAuth callback simulation set up. The GoogleAuthCallback component should now trigger.");
