...
exports.createInvoice = async (req, res) => {
  try {
    ...
    if (!items || !Array.isArray(items) || items.length === 0) {
      return res
        .status(400)
        .json({ msg: "At least one invoice item is required" });
    }
  ...
  try {
    ...
    const trackingPixel = `
      <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #eee; color: #999; font-size: 12px; text-align: center;">
        <p>This email was sent via WorkSage - Your Business Management Solution</p>
        <img src="${
          process.env.BACKEND_URL || "http://localhost:5000"
        }/api/invoices/track/${invoice.trackingId}" 
  ...
  try {
    ...
    await createNotification({
      ...
      data: {
        ...
        payment: { amount, tip, note, method: "manual" },
      },
    });
