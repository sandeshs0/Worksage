...
const PLAN_FEATURES = {
  ...
  pro: {
    ...
    storage: '10GB',
    collaborators: 10,
    features: ['Advanced project management', 'Priority support', 'Advanced templates', 'Time tracking', 'Reports']
  },
  vantage: {
    projects: 'Unlimited',
    storage: '100GB',
    collaborators: 'Unlimited',
    features: ['Enterprise project management', '24/7 support', 'Custom templates', 'Advanced analytics', 'API access', 'White-label options']
  }
};
...
exports.initiatePlanUpgrade = async (req, res) => {
  try {
    ...
    if (existingUpgrade && !existingUpgrade.isExpired()) {
      return res.status(400).json({
        success: false,
        message: 'You already have a pending plan upgrade. Please complete or cancel it first.',
        existingUpgrade: {
    ...
    const khaltiPayload = {
      ...
      amount: upgradeAmount * 100, // Convert to paisa (NPR * 100)
      purchase_order_id: purchase_order_id,
      purchase_order_name: `Worksage ${targetPlan.toUpperCase()} Plan Upgrade`,
      customer_info: {
      ...
      amount_breakdown: [
        {
          label: `${targetPlan.toUpperCase()} Plan`,
          amount: PLAN_PRICES[targetPlan] * 100
        },
        {
          label: `Current ${currentPlan.toUpperCase()} Plan Credit`,
          amount: -PLAN_PRICES[currentPlan] * 100
        }
      ],
      product_details: [
        {
          identity: `plan_${targetPlan}`,
          name: `Worksage ${targetPlan.toUpperCase()} Plan`,
          total_price: upgradeAmount * 100,
          quantity: 1,
          unit_price: upgradeAmount * 100
        }
      ],
    ...
    planUpgrade.paymentDetails.amount_breakdown = khaltiPayload.amount_breakdown;
    await planUpgrade.save();
    ...
    try {
      ...
      console.error('Khalti initiate payment error:', khaltiError.response?.data || khaltiError.message);

      // Update plan upgrade status to failed
      planUpgrade.status = 'failed';
      planUpgrade.failureReason = khaltiError.response?.data?.detail || khaltiError.message;
      await planUpgrade.save();
...
exports.handleKhaltiCallback = async (req, res) => {
  try {
    ...
    if (!planUpgrade) {
      console.error('Plan upgrade not found for purchase_order_id:', purchase_order_id);
      return res.redirect(`${process.env.FRONTEND_URL}/payment/error?reason=upgrade_not_found`);
    }
    ...
    if (status === 'User canceled' || status === 'Expired' || status === 'Failed') {
...
exports.verifyPlanUpgrade = async (req, res) => {
  try {
    ...
    if (paymentData.status === 'Completed' && planUpgrade.status !== 'completed') {
